...
type t = { hs_code : Hs_code.t; origin : Country.t option; cost : Money.t }
...
let parse_country_opt s =
  ...
  | raw_string -> (
      match Country.of_iso_string raw_string with
      | Ok c -> Ok (Some c)
      | Error _ -> (
          match Country.of_string raw_string with
          | Ok c -> Ok (Some c)
          | Error e -> Error e))
...
let of_strings_exn hs_code_str origin_str cost_str =
  match of_string hs_code_str origin_str cost_str with
...
let print output_stream t =
  let origin_str =
    match t.origin with
    | None -> "Unknown"
    | Some c -> Country.to_iso_string c
  in
  Format.fprintf output_stream "Material { hs_code = %s; origin = %s; cost = %s }" 
    (Hs_code.to_string t.hs_code)
    origin_str
    (Money.to_string t.cost)