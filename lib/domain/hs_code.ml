...
let of_string s =
  let cleaned = String.trim s |> String.to_seq |> Seq.filter (fun c -> c <> '.') |> String.of_seq in
  let len = String.length cleaned in

  let is_numeric str = String.to_seq str |> Seq.for_all (function '0' .. '9' -> true | _ -> false) in
